...
def tc_info():
    ...
    other_test_report = """
        ...
        Matched Result:
        --------------
        pattern: (?P<file_type>\\S)(?P<file_permission>\\S+) (?P<hard_links>\\d+) (?P<file_owner>[a-zA-Z0-9]+) (?P<file_group>[a-zA-Z0-9]+) +(?P<file_size>\\d+) (?P<date>[a-zA-Z]{3} +\\d{1,2}) (?P<time>\\d+:\\d+) (?P<filename>[\\x21-\\x7e]+( [\\x21-\\x7e]+)*)
